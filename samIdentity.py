...
parser.add_argument('-m', '--matches', 
                    help='min num of matches to return', type=int, default=0)
...
def makeHeader():
	...
	rtn += "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("matches", "mismatches", 
			"insertions", "deletions", "insertion_events", "deletion_events", "strand")
	return(rtn)
...
def formatRead(read):
    rtn = ""
    rtn += "{}\t{}\t{}\t{}\t".format(read.query_name,
                                     read.query_alignment_start,
                                     read.query_alignment_end,
                                     read.infer_query_length() )
    rtn += "{}\t{}\t{}\t".format(read.reference_name,
    ...
    if read.is_reverse:
        strand="-"
    bymatches, byevents, byall = perId(matches,
                                       mismatch,
                                       ins,
                                       dele,
                                       insEvent,
                                       delEvent)
    rtn += "{}\t{}\t{}\t".format(bymatches,
                                 byevents,
                                 byall )
    rtn += "{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(matches,
                                                 mismatch,
